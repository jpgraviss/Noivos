...
**Known, not-yet-closed gap**: decorative icons (most `lucide-react-native` icons placed next to their own text label) aren't marked `aria-hidden` — in practice this is low-severity (an `<svg>` with no accessible-name-conferring attribute is typically skipped by default AT heuristics), but it wasn't verified exhaustively and would be a reasonable follow-up if a future accessibility audit has time for it.
