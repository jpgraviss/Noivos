...
| `0006_add_partnership_invite_accept_flow.sql` | ⬜ **Not yet applied.** Until this runs, `/api/partnership/accept` will fail RLS on every attempt (the invitee has no way to read or update the pending invite row), so nobody can actually accept an invite yet even though the UI/API for it is live. |
