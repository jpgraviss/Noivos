...
**New open item:** the founder asked mid-session ("Let's use Ollama as our AI") to consider self-hosted Ollama instead of the OpenAI Responses API confirmed in §6/§6.2 — flagged rather than silently swapped, since Ollama's self-hosted/local-server model doesn't fit the Vercel serverless topology (Backend Architecture §1) the same way a hosted API does, and would need real infrastructure (a dedicated always-on host, not a Vercel function) to run in production. Not yet decided; see the Open Questions section for the question put to the founder directly.
