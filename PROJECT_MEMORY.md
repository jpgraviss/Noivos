...
8. **`packages/ui` `Card` glow deprecation.** The glow-based elevation effect (Design System §6) uses react-native `shadow*` style props, which react-native-web logs as deprecated in favor of `boxShadow`. Cosmetic only (no visual or functional break observed), not fixed as part of this change — low-priority cleanup whenever `packages/ui` is next touched.
